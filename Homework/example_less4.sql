...
-- (используя UNION)
SELECT initiator_user_id as 'result id'
FROM friend_requests t1
WHERE target_user_id = 1 AND status = 'approved'
AND 
	(
		SELECT COUNT(*)
        FROM friend_requests t2
        WHERE status <> 'approved' AND t1.updated_at < t2.updated_at
        AND t1.initiator_user_id = t2.target_user_id
        AND t1.target_user_id = t2.initiator_user_id
    ) = 0
UNION
SELECT target_user_id as 'result id'
FROM friend_requests t1
...
SELECT 
    ...
    u.firstname, 
    u.lastname, 
    IFNULL(p.hometown,  "Отсутсвует информация о городе"),
    IFNULL(m.filename, "Отсутсвует фото")
FROM users u
