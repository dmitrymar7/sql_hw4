...
SET @cat_name = "Murzik"; -- "Мурзик" - по условию, но проверим на существующем в таблице имени

SELECT shops.*
FROM shops
INNER JOIN cats
ON shops.id = cats.shops_id
WHERE cats.name = @cat_name;
...
AND DATE(orders.ord_datetime) BETWEEN @date AND DATE_ADD(@date, interval 1 WEEK);